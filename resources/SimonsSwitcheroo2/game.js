...
class GameManager {
    ...
    report() {
        ...
        PS.debug("Rotation: "+this.rotation+"\n")
    }
    ...
    generateSequence(length) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            let cellType = PS.random(cellTypeCount)
            if (cellType === 5 && length >= 6) {
                this.sequence.push(new OppositeCell(this.idCounter % 8, Math.floor(this.idCounter / 8), OPP, this))
            }
            else if (cellType === 6 && length >= 8) {
                this.sequence.push(new RotateCell(this.idCounter % 8, Math.floor(this.idCounter/8), 1, this))
            }
            else if (cellType === 7 && length >= 10) {
                this.sequence.push(new RotateCell(this.idCounter % 8, Math.floor(this.idCounter/8), -1, this))
            }
            else {
...
class InputCell extends Cell {
    ...
    checkInput(key) {
        let rightKey = this.desiredKey + this.gameManager.rotation
        if (rightKey > 1008) {
            rightKey = 1005 + ((this.gameManager.rotation % 4)-1)
        }
        else if (rightKey < 1005) {
            rightKey = 1008 + ((this.gameManager.rotation % 4)+1)
        }
...
class RotateCell extends Cell {
    constructor(x, y, direction, gameManager) {
        let glyph = "0"
        if (direction === 1) {
            glyph = ROTF
        }
        ...
        super(x, y, PS.COLOR_YELLOW, glyph, gameManager)
        this.direction = direction
        this.blit()
    }
    ready() {
        if (this.direction === 1) {
            this.gameManager.rotation++;
