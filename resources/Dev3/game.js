...
function rowAndColumnBorders(x, y) {
    PS.border(PS.ALL, PS.ALL, 8)
    PS.borderColor(PS.ALL, PS.ALL, PS.COLOR_GRAY)
    PS.glyph(PS.ALL, PS.ALL, 0)
    if (mode === ROW) {
        PS.borderColor(0, y, PS.COLOR_RED)
        PS.glyph(0, y, "←")
        PS.borderColor(gridWidth - 1, y, PS.COLOR_GREEN)
        PS.glyph(gridWidth - 1, y, "→")
    }
    else if (mode === COLUMN) {
        PS.borderColor(x, 0, PS.COLOR_RED)
        PS.glyph(x, 0, "↑")
        PS.borderColor(x, gridHeight - 1, PS.COLOR_GREEN)
        PS.glyph(x, gridHeight - 1, "↓")
    }
}
...
PS.enter = function( x, y, data, options ) {
    ...
    else if (y != mouseCoords[1]) {
        mode = COLUMN
    }
    mouseCoords = [x, y]
    rowAndColumnBorders(x, y)
